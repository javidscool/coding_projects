 ...
 *
 * ThreadsState stores the list of Threads indexed by id USED in `entities`, as well
...
export interface ThreadsEntities {
	//map TYPE of thread ID to Thread
	[id: string]: Thread;
}
...
export const getAllThreads = createSelector(
	getThreadsEntities,
	( entities: ThreadsEntities ) => Object.keys(entities).map(
...
export const getAllMessages = createSelector(
	...
	( threads: Thread[] ) =>
		...
		[])
		.sort((m1, m2) => m1.sentAt - m2.sentAt)); // sort them by time, with negative number of result sorting m1 first