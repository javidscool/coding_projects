...
class RayTracer{
    ...
    public:
        ...
        RayTracer(std::vector<Object> objList, std::vector<BVH> bvhList, const int width, const int height);
        ...
        bool IntersectBVHNode(const AABBox &bounds, Eigen::Vector3f invDir, Eigen::Vector3i dirIsNeg, Eigen::Vector3f rayOrig, float rayMaxt);

        bool IsInBox(const AABBox &bounds, Eigen::Vector3f pt);

        //shit for adaptive supersampling
        Color AdaptiveSuperSamp(MyRay originRay, int j, int i);

        std::vector<MyRay> GetAdapRays(MyRay middleRay, int j, int i);

        bool IsColorTooDiff(Color color1, Color color2);

        Color AdaptiveSuperPass2(MyRay ray1, MyRay ray2, Color ray1Col, Color ray2Col, int situation);
