...
/// The shared struct represents data that is shared between all peers 
/// in the chat server.
///
...
impl Lines {
    ...
    /// Buffer a line.
    ///
    /// This writes the line to an internal buffer (wr). Calls to `poll_flush` will
    ...
    fn fill_read_buf(&mut self) -> Poll<(), io::Error> {
        loop {
            ...
            // if there's no more data left to read, we're ready
...
// Stream is a trait we're implementing for Lines to process lines in a stream
impl Stream for Lines {
    ...
    fn poll(&mut self) -> Result<Async<Option<Self::Item>>, Self::Error> {
        ...
        // off the socket...fill_read_buf() will put data from the socket into rd
        //
        ...
        // we go thru two bytes at a time with windows(2), which is what we want cause we're 
        // trying to find "\r\n", then we enumerate() over those bytes, get
        // an index location and value from enumerate(), pass those two to find(),
        // which ignores the index and tries to find where the bytes match a newline,
        // and if we find a newline, return the position with map(), which gives a Some(val)
...
struct Peer {
    ...
    ///
    /// The socket address is used as the key in the `peers` HashMap of the Shared struct. The
    /// address is saved here as well so that the `Peer` drop implementation can clean up its
    /// entry in the Shared peer's hashmap.
    addr: SocketAddr,
}
...
impl Future for Peer {
    ...
    fn poll(&mut self) -> Poll<(), io::Error> {
        ...
        // First we receive all messages from peers and write them to our write buffer in lines.
        ...
        // Now we read new lines from the socket
...
fn process(socket: TcpStream, state: Arc<Mutex<Shared>>) {
    // Wrap the socket with the `Lines` codec that we wrote above in order to 
    // process the socket line by line.
    //
    // By doing this, we can operate at the line level instead of doing raw byte
    // manipulation.
    let lines = Lines::new(socket);

    // connection below is a future we're gonna spawn on a seperate task
    ...
    // of `(first, rest)` where `rest` is the original stream instance minus the first item.
    // into_future() calls lines.poll() one time to get the first element from the stream, the name
...
// main() returns a potential error because we try to parse an IP address and 
// bind it to the TcpListened and if we can't we return error from main()
// if nothing is wrong main() return Ok(()) at the bottom
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // will return error from main if this doesn't work
    ...
    // if we got this far, return Ok(()) from main()
    Ok(())
}
