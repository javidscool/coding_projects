...
//	chromosome
//-----------------------------------------------------------------
void TravSMan::InsertionMutation(std::vector<int> &chromo){
...
// defined in the text
//-------------------------------------------------------------------
void TravSMan::PMX(const vector<int> &parent1, 
...
void TravSMan::Generation(){
	...
	CreateRandomImmigrants( REPLACEMENT_RATE, popSize );
	...
	while (vecNewPop.size() != popSize){
		...
		//perform mutation
		InsertionMutation(child1.vecCities);
		InsertionMutation(child2.vecCities);
...
void TravSMan::CreateRandomImmigrants( double ReplaceRate, int max){

	//initialize the Rnadom Immigrants Ppoluation Vector
	vector<int> randomPop;

	randomPop = GrabRandomCities( ReplaceRate * max, max );

	for (int i = 0; i < (ReplaceRate*max); i++){
		population.erase(population.begin()+randomPop[i]);
		population.insert(population.begin()+randomPop[i], TSMPath(numCities) );
	}

	cout << "Random Immigrants was used" << endl;
}

vector<int> TravSMan::GrabRandomCities(int numCities, int maximum){
	...
	for (int i = 0; i < numCities; i++ ){
		//we use numCities-1 because we want ints numbered from zero
		int NextPossibleNumber = RandInt(0,maximum-1);

		while(TestNumber(vecPerm, NextPossibleNumber))
		{
			//give me another random number if the previous one is already in the lsit
			NextPossibleNumber = RandInt(0, maximum-1);
		}
