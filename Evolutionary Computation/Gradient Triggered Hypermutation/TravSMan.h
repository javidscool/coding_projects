...
struct TSMPath{
	...
	TSMPath(int numCities) : fitScore(0.0), mutRate(0.0)
	{
...
private:
	...
	//Way to store Previous Fitnesses
	double BestFitnessArray[5];
	...
	void InsertionMutation(vector<int> &chromo, double vMutationRate);
...
public:
	...
	TravSMan(double mutation_rate,
			 double hypermutation_rate,
			 double crossover_rate,
			 int pop_size,
			 int num_cities,
			 ChangeType t
			 ) : mutationRate(mutation_rate),
				 hyperMutationRate(hypermutation_rate),
	...
	double AdaptiveMechanism(double currentBestFitness);
